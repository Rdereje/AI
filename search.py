...
class Node_Sudoku:
    ...
    def child_node(self, problem, action, r, c):
        """[Figure 3.10]"""
        next_state = problem.result(copy.deepcopy(self.state), r, c, action)
        next_node = Node_Sudoku(next_state, self, action,
                         problem.path_cost(self.path_cost, self.state,
                                           action, next_state), r, c)
        return next_node
    ...
    def playable(self):
        ...
        size = len(self.state)
        for i in range(size):
            if self.state[self.r][i] == value and i != self.c:
                return False
            if self.state[i][self.c] == value and i != self.r:
                return False
        if size == 4:
            div = 2
        else:
